...
docker build -t ubuntu-network -f Dockerfile_ubuntu-network .
echo -e "${BGREEN}\n[+]${NC} Building kali image...${NC}" 
docker build -t kali -f Dockerfile_kali .
echo -e "${BGREEN}\n[+]${NC} Building suitecrm-server image...${NC}"
docker build -t suitecrm-server -f Dockerfile_suitecrm-server .
